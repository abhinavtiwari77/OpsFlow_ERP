...
import { fetchChallans, KEYS } from "../api/queries";
...
export function Challans() {
  ...
  const { data: items = [], isLoading, isError, error, refetch } = useQuery({
    queryKey: [...KEYS.challans, status],
    queryFn: () => fetchChallans(status),
    enabled: !!user,
    ...
    <div>
      ...
      {isError && (
        <div className="error-banner">
          {(error as any)?.response?.data?.error || (error as any)?.message || "Failed to load challans"}
        </div>
      )}
